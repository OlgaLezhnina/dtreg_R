...
To load a DTR schema, you need to know its identifier (see the help page XXX), such as the datatype DOI for the ePIC or the template URL for the ORKG.

For instance, you intend to use the "statistical test of difference" schema; it has the ePIC DOI "https://doi.org/21.T11969/ff5e3f857788d20dd1aa".
...
In addition to the schema you requested (in this case, the "statistical test of difference"), you get schemata that you might need for using it.
...
The help page XXX specifies which fields to use, and what input is required by a field (e.g., another schema or a specific type of data, such as numeric).
...
The result should be either one of the [basic R data types](https://www.w3schools.com/r/r_data_types.asp) or an [R factor](https://www.w3schools.com/r/r_factors.asp).

By default, dtreg assigns a data frame a generic name "Table". If you want to give your data frame a name, you can use a tuple. Please always include first the data frame and then the name as a string in the tuple. 
...
For this example, we use the statistical_test_of_difference datatype from the ePIC (see [Load a DTR schema](#section1) above).  
