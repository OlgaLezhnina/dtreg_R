...
To load a DTR schema, you need to know its identifier, such as the ePIC datatype DOI or the ORKG template URL (see the [help page](https://orkg.org/help-center/article/47/reborn_articles)). For example, to report Wilcoxon test, you need the group_comparison schema, which has the ePIC DOI "https://doi.org/21.T11969/b9335ce2c99ed87735a6". If a valid identifier is used, you get an R object containing information about the DTR schema:
...
However, the data you want to write are usually more complex than this. While creating an instance, please pay attention to fields you use and types of input required by these fields (e.g., another schema or a specific type of data, such as numeric).
...
In this vignette, we show reporting data analysis results with dtreg. The following code was simplified for the illustration purposes: 
...
regr <- stats::lm(Petal.Length ~ Petal.Width, data = virg)
res_regr <- summary(regr)
```

Here, we selected virginica and versicolor rows of the Iris dataset as two separate data frames; conducted Wilcoxon test comparing petal length of these species; and explored the relationship between petal length and petal width in virginica with simple linear regression (SLR).  

### Choose parts of analysis to address and schemata to load

First of all, we select schemata based on what analytic methods we report. Let us omit preprocessing, which is rather trivial in this case, and focus on Wilcoxon test and SLR. For them, as the [help page](https://orkg.org/help-center/article/47/reborn_articles) specifies, we need group_comparison and the regression_analysis schemata. Also, for any data analysis, the overarching data_analysis schema is required. Let us load these (see [Load a DTR schema](#section1) above):
...
Now we can prepare to writing the instances.  

### Prepare data frames with test results 

Reporting the data analysis results transparently is important, and there is a trade-off between concise and comprehensive presentation. For Wilcoxon test, this data frame will suffice:     
...
For SLR, we have more information from the summary object that is crucial to report:

```{r}
regr_coeff <- data.frame(res_regr$coefficients)
regr_model <-
  data.frame(
    F = 5.557,
    numdf = 1,
...
```
Please remember that this is a simplistic example, and in real tests you might want to report more details (e.g., the effect size for Wilcoxon test).

### Find out which parts can be reused

Schemata such as the data_item or software_method are used in all analytical schemata as their parts, and when an instance of such a class is converted into JSON-LD format, it does not matter, which larger analytical schema it belongs. It can be easily checked:
...
Therefore, some parts of the Wilcoxon instance can be reused in the SLR instance. These are (i) the data_item instance describing the "virg" dataset; (ii) the software_library instance describing software R and package "stats"; and (iii) the component instance indicating that our target variable is "Petal.Length":     

```{r}
data_virg <- dt_wilc$data_item(
  label = "virg",
  has_characteristic = dt_wilc$matrix_size(number_of_rows = 50,
                                           number_of_columns = 5)
)
...
instance_wilc <- dt_wilc$group_comparison(
  ...
  has_input = list(data_vers, data_virg),
...
```{r}
soft_method_regr <-
  dt_regr$software_method(
    label = "stats::lm",
    part_of = soft_library,
    is_implemented_by = paste(
      "regr = stats::lm(Petal.Length ~ Petal.Width, data = virg)",
      "res_regr = summary(regr)",
      sep = "\n"
    )
...
```
A new R package based on dtreg will pack this code into just a couple of lines for simplicity (you can look for the mrap package in CRAN soon). However, writing the results with dtreg as described here has its benefits, as it provides you with freedom of choice in presenting your results. 
