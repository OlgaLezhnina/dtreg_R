...
Here, we took the Iris dataset, selected rows with virginica and versicolor as two separate dataframes, and conducted Wilcoxon test comparing their petal length. Thus, we need to load (see [Load a DTR schema](#section1) above) the data_analysis ePIC datatype for the overall report, the data_preprocessing for selecting rows, and the group_comparison for Wilcoxon test:
...
Let us write down the preprocessing instance. Here, the input is the initial Iris dataset, the output consists of two resulting datasets, and the software method is described without specifying a software library, as we were merely selecting rows.  

```{r}
software <- dt_prep$software(label = "R",
                             versioninfo = "4.3.1")
soft_method <- dt_prep$software_method(
  label = "select rows",
  part_of = software,
  is_implemented_by = "virg <- iris[iris$Species == 'virginica',]/n
vers <- iris[iris$Species == 'versicolor',]"
)

dim_iris <- dt_prep$matrix_size(number_of_rows = 150,
                                number_of_columns = 5)

input_prep <- dt_prep$data_item(label = "Iris dataset",
                                has_characteristic = dim_iris,
                                source_url = "https://search.r-project.org/CRAN/refmans/MVTests/html/iris.html")

dim_virg <- dt_prep$matrix_size(number_of_rows = 50,
                                number_of_columns = 5)
dim_vers <- dt_prep$matrix_size(number_of_rows = 50,
                                number_of_columns = 5)

data_virg <- dt_prep$data_item(label = "virginica",
                               has_characteristic = dim_virg)
data_vers <- dt_prep$data_item(label = "versicolor",
                               has_characteristic = dim_vers)
instance_prep <-
  dt_prep$data_preprocessing(
    label = "select rows virginica and versicolor",
    executes = soft_method,
    has_input = input_prep,
    has_output = c(data_virg, data_vers)
  )
```

Let us now create an instance for Wilcoxon test. Here, we indicate the software library that we used to conduct the test, and we need to specify the target (dependent) variable. The inputs are two datasets created in the previous step, and the output is the result of Wilcoxon test.     
...
soft_method <-
  dt_wilc$software_method(label = "Wilcoxon rank sum test",
                          ...
                          is_implemented_by = "stats::wilcox.test(vers$Petal.Length, virg$Petal.Length)")
...
instance_wilc <- dt_wilc$group_comparison(
  ...
  executes = soft_method,
  has_input = c(data_virg, data_vers),
...
instance_all <- dt_all$data_analysis(
  ...
  has_part = c(instance_prep, instance_wilc)
)
```
